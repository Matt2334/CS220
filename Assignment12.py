...
#Assignment12: Trees and Tree Algorithms
...
def print_tree_properties(desc, tree, properties):
    data = [[prop.__name__, prop(tree)] for prop in properties]
    headers = ["Property Name", "Value"]
    alignments = ["l"] * 2
    as8.print_table(desc, headers, data, alignments)
...
def do_tree(assn, desc, keys):
    ...
    matrix = tree_to_matrix(len(keys), edges)
    as11.do_graph(assn, matrix, desc, True)
    properties = [height, min_key, max_key, inorder, preorder, postorder, reverseorder]
