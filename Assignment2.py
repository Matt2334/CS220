...
# You may want to reverse the binary string (using binary[::-1]) before converting it to a decimal number, as that may better align the result with the function number.

# Define a function func_body(f) that returns the content of the function. This will be used later in the truth table.
def func_body(f):
...
def f2_converse_non_impl(p, q):
    return b_and(b_not(p), q)

def f3_negation_p(p, q):
    return b_not(p)

def f4_nimply(p, q):
    return b_and(p, b_not(q))

def f5_negation_q(p, q):
    return b_not(q)
...
def analyze_functions():
    functions = [f0_false, f1_nor, f2_converse_non_impl, f3_negation_p, f4_nimply, f5_negation_q, f6_xor, f7_nand,
                     f8_and, f9_xnor, f10_q, f11_imply, f12_p, f13_converse, f14_or, f15_True]
    data=[]
    for f in functions:
        results=analyze_boolean_table(f)
        row=[f.__name__, func_body(f), str(results[6]), results[7], reverse_engineer(f)]
        data.append(row)
    headers = ["Name","Function", "Binary", "Decimal", "Reversed"]
    print_table("All Binary Functions", headers, data, ["l", "l", "r", "r", "l"])


# [6] Define a function that, using the method discussed in class, reverse-engineers a 0/1 boolean table into a
# function that yields that table's output column.
def reverse_engineer(f):
    tt, tt_heads, tt_rows, tt_cols, tt_vars, tt_out, tt_bin, tt_dec= analyze_boolean_table(f, False)
    function = " + ".join(["".join([tt_heads[j] + ("'" if tt.iloc[i][j] == 0 else "") for j in range(tt_vars)]) for i in range(tt_rows) if tt_out[i] == 1])
    return function


# [7] Write a function half_adder(x, y) that uses the functions created in Task [1] as "gates" and produces
# the s (sum) and c (carry) outputs per the table in the slides.
def half_adder():
